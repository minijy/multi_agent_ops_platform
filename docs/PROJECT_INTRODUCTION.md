...
非管理员不展示运行概览、审批中心、连接器、长期记忆、用户权限、审计和系统设置等管理页面。
...
- 本地向量、PostgreSQL/pgvector 和 Qdrant 语义检索。
...
开发环境可以使用 SQLite 快速启动，生产或多实例环境建议使用 PostgreSQL 保存控制面、Session 事件、
记忆和队列状态。
...
当前代码库包含 211 个通过的自动化测试，另有 3 个根据部署依赖跳过的测试。测试覆盖：
...
5. 评测与质量闭环：建立更完整的业务正确性、权限攻防和长任务稳定性评测。
