...
function renderMemoryItems(items){
  ...
  $$('[data-memory-correct]').forEach(button=>button.addEventListener('click',async()=>{const current=state.memories.find(item=>item.id===button.dataset.memoryCorrect);const content=await askDialog({title:'纠正记忆',message:'输入纠正后的内容。',input:true,required:true,okLabel:'保存',prefill:current?.content||''});if(content)await memoryAction(`/v1/memories/${encodeURIComponent(button.dataset.memoryCorrect)}/correct`,{method:'POST',body:JSON.stringify({content})},'记忆已纠正')}));
  $$('[data-memory-delete]').forEach(button=>button.addEventListener('click',async()=>{if(await askConfirm('确定删除并擦除这条记忆内容？',{title:'删除记忆',okLabel:'删除',danger:true}))await memoryAction(`/v1/memories/${encodeURIComponent(button.dataset.memoryDelete)}`,{method:'DELETE',body:JSON.stringify({reason:'admin_requested'})},'记忆已删除')}));
